...
class MPII:
    ...
    def _save_joints(self):
        ...
        if os.path.exists(joint_data_fn):
            ...
            return
        annotations_path = os.path.join(self.path, 'mpii_human_pose_v1_u12_1.mat')
        mat = loadmat(annotations_path)

        fp = open(joint_data_fn, 'w')

        for i, (annotation, train_flag) in enumerate(
                zip(mat['RELEASE']['annolist'][0, 0][0],
                    mat['RELEASE']['img_train'][0, 0][0])):

            img_fn = annotation['image']['name'][0, 0][0]
            train_flag = int(train_flag)

            if 'annopoints' in str(annotation['annorect'].dtype):
                annopoints = annotation['annorect']['annopoints'][0]
                head_x1s = annotation['annorect']['x1'][0]
                head_y1s = annotation['annorect']['y1'][0]
                head_x2s = annotation['annorect']['x2'][0]
                head_y2s = annotation['annorect']['y2'][0]
                for annopoint,\
                    head_x1,\
                    head_y1,\
                    head_x2,\
                    head_y2 in zip(annopoints,
                                   head_x1s,
                                   head_y1s,
                                   head_x2s,
                                   head_y2s):
    ...
    def _train_test_split(self, image_paths, tfrecord_paths):
        print('Splitting data to train-test subsets ...')
    ...
    def _make_gaussian(self):
        size = int(self.gaussian_sigma * self.variance)
    ...
    def _generate_believes(self, h, w, joint_pos):
        ...
        for i, joint in enumerate(joint_pos):
            for person in joint:
                gaussian_map = self._generate_gaussian_map(gaussian, h, w, person[0], person[1])
                believes[:, :, i] += gaussian_map
        return believes
