...
class CPM:
    ...
    def _cpm_first_stage(self, features):
        x = self._conv2d(features, filters=256, kernel_size=(9, 9))
    ...
    def _cpm_middle_stage(self, features, former_believes, prefix):
        ...
        y = self._conv2d(x, filters=256, kernel_size=(11, 11))
    ...
    def _feature_extractor(self, input_image):
        y = self._conv2d(input_image, filters=64, kernel_size=(9, 9))
        x = self._add_skip_connection(input_image, y)
        x = tfkl.MaxPooling2D((2, 2), strides=1, padding='same')(x)

        y = self._conv2d(x, filters=128, kernel_size=(9, 9))
        x = self._add_skip_connection(x, y)
        x = tfkl.MaxPooling2D((2, 2), strides=1, padding='same')(x)
        ...
        y = self._conv2d(x, filters=256, kernel_size=(5, 5))
        x = self._add_skip_connection(x, y)
        return x
